...
class EntanglementGenerationA(EntanglementProtocol):
    ...
    def received_message(self, src: str, msg: EntanglementGenerationMessage) -> None:
        ...
        if msg_type is GenerationMsgType.NEGOTIATE:  # primary -> non-primary
            # configure params
            # Note again, that all of these correspond to the delays to the BSM node.
    ...
    def entanglement_generation_trajectree(self):
        ...
        if self.owner.cached_entanglement.get(self.remote_node_name, None) == None:
            ...
            params = {
                "PA_det_eff": 1-1e-3,
                "BSM_det_loss_1": CONFIG["templates"]["perfect_bsm"]["TrajectreeBSM"]["detectors"][0]["efficiency"], #  0.045,
                "BSM_det_loss_2": CONFIG["templates"]["perfect_bsm"]["TrajectreeBSM"]["detectors"][1]["efficiency"], # 0.135,
                "BSM_dark_counts_1": 1+CONFIG["templates"]["perfect_bsm"]["TrajectreeBSM"]["detectors"][0]["dark_count"],
                "BSM_dark_counts_2": 1+CONFIG["templates"]["perfect_bsm"]["TrajectreeBSM"]["detectors"][1]["dark_count"],
                "channel_loss": 0, # 1 - 10 ** (CONFIG["qconnections"][0]["distance"] * CONFIG["qconnections"][0]["attenuation"] / -10),
                "chi": CONFIG["templates"]["perfect_router"]["chi"], # Here we are assuming that all the nodes in the network have the same mean photon number.
            ...
            # First, we calculate the probabilty of a succesful entanglement generation operation, without the post-selection.
            fidelities, probabilities, t_eval = perform_swapping_simulation(N, num_modes, num_simulations, params, error_tolerance)

            # Next, to caluclate fidelity of the generated entanglement, we calulate the visibility of the shared entanglement and calculate the fidelity froom that. 
            fidelity = self.calculate_post_selection_fidelity(params)    
            
            self.owner.cached_entanglement[self.remote_node_name] = (np.mean(probabilities), fidelity)
        else:
    ...
    def calculate_post_selection_fidelity(self, params):

        params['if_analyze_entanglement'] = True
        params['calc_fidelity'] = False
        ...
        params["delta_list"] = np.array([np.pi/2])

        params["alpha_list"] = np.array([np.pi/2])
        _, probabilities, _ = perform_swapping_simulation(N, num_modes, num_simulations, params = params, error_tolerance = error_tolerance)
        ...
        visibility = (prob_same_phase - prob_diff_phase) / (prob_same_phase + prob_diff_phase)
        fidelity = (3*visibility + 1)/4
        return fidelity
